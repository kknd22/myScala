    ...
    if (n == 0) xs
      ...
      def merge(xs: List[Int], ys: List[Int], accu: List[Int]): List[Int] =
        (xs, ys) match {
          case (Nil, Nil) => {
            val t = accu.tail
            println("0 accu: " + accu.mkString(", ") )
            accu.reverse
          }
          case (Nil, y :: ys1) =>
            println("1 accu: " + accu.mkString(", "))// + " - id: " + accu)
            val res = merge(xs, ys1, y :: accu)
            res
          case (x :: xs1, Nil) =>
            println("2 accu: " + accu.mkString(", "))// + " - id: " + accu)
            val res = merge(xs1, ys, x :: accu)
            res
          case (x :: xs1, y :: ys1) =>
            if (x < y) {
              println("3 accu: " + accu.mkString(", "))// + " - id: " + accu)
              val res = merge(xs1, ys, x :: accu)
              res
            }
            else {
              println("4 accu: " + accu.mkString(", "))// + " - id: " + accu)
              val res =merge(xs, ys1, y :: accu)
              res
            }
        }
